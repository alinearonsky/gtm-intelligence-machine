  ...
  return (
    <main className="mx-auto max-w-3xl space-y-8 p-6">
      ...
      <section className="space-y-4">
        {rules.rules.map((r) => (
          <div key={r.id} className="rounded border p-4 space-y-2">
            ...
            <details className="text-xs">
              ...
              <pre className="mt-2 overflow-x-auto rounded bg-muted p-2">{JSON.stringify(r.when, null, 2)}</pre>
            </details>
          </div>
        ))}
      </section>
      ...
      <section className="space-y-2">
        <h2 className="text-lg font-semibold">Product lens — {lens.label}</h2>
      ...
      <section className="space-y-2">
        <h2 className="text-lg font-semibold">Role ontology</h2>
        <ul className="space-y-1 text-sm">
